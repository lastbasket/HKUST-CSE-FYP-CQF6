...
class BaseDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.img_dir = os.path.join(BASE_FOLDER, 'StanExtV12_Images')
        self.jsonfile = os.path.join(BASE_FOLDER, 'labels', config.JSON_NAME[dataset])  # accessing new version of keypoints.json
        # create train/test split
        with open(self.jsonfile) as anno_file:
            self.anno = json.load(anno_file)

        self.data_idx = np.load(os.path.join(config.DATASET_FILES[is_train][dataset]))[0:32]

        print("train field:", field)
        if field:
            self.img_idx = img_idx
            self.data_idx = self.data_idx[img_idx:img_idx+1]
        full_len = len(self.data_idx)
        # self.data_idx = self.data_idx[0:full_len//3]
        print("Number of images: {}".format(len(self.data_idx)))
    ...
    def j2d_processing(self, kp, center, scale, r, f):
        ...
        if f:
            kp = flip_kp(kp, config.IMG_RES)
        kp_norm = kp.copy()
        # print("kp",kp)
        # convert to normalized coordinates
        kp_norm[:, :-1] = 2. * kp_norm[:, :-1] / self.img_res - 1.
        # print("kp_norm", kp_norm)

        kp = kp.astype('float32')
        return kp, kp_norm.astype('float32')
    ...
    def __getitem__(self, index):
        ...
        filename = imgname_raw.split('/')[1].split('.')[0] if self.dataset == 'stanford' else imgname_raw.split('.')[0]
        assert os.path.exists(imgname), "Cannot find image: {0}".format(imgname)
        img = cv2.imread(imgname)[:, :, ::-1].copy().astype('float32')

        seg = seg_from_anno(a)  # (H, W) bool
        seg = seg.astype('float32') * 255.
        assert (np.nonzero(seg)[0].shape[0] > 1)
        ...
        kp_S24, kp_S24_norm = self.j2d_processing(kp_S24, center, sc * scale, rot, flip)

        kp_S24 = torch.from_numpy(kp_S24)
        kp_S24_norm = torch.from_numpy(kp_S24_norm)

        # print(img.min().item(), img.max().item())
        # exit()
        img_crop = self.rgb_processing(img, center, sc * scale, rot, flip, pn, border_grey_intensity=255.0)
        seg_crop = self.rgb_processing(seg, center, sc * scale, rot, flip,
                                       np.array([1.0, 1.0, 1.0]))  # No pixel noise multiplier
        ...
        item['keypoints_norm'] = kp_S24_norm[config.EVAL_KEYPOINTS]
        item['keypoints'] = kp_S24[config.EVAL_KEYPOINTS]
