...
class MeshModel(nn.Module):
    def __init__(self, device, shape_family_id, betas_scale=False, shape_init=None, render_rgb=False, color=False, use_batch=True):
        '''
        Args:
        ...
        if shape_init == 'smal':
            ...
            shape_init = None
        self.color = color
        # print('faces: ', self.smal.faces.shape)
        self.meshnet = MeshNet_img(input_size, betas_scale=betas_scale, norm_f0=config.NORM_F0, nz_feat=config.NZ_FEAT,
                                   shape_init=shape_init, faces_size=self.smal.faces.shape[0], color=self.color, use_batch=use_batch)
        print('INITIALIZED')

    def forward(self, inp):

        if self.color:
            pred_codes, textures = self.meshnet(inp)
            return pred_codes, textures
        else:
