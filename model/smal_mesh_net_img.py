...
class ResNetConv(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if n_blocks >= 2:
            ...
            img_feat_multiscale.append(torch.nn.Upsample(size=(h//4, w//4), mode='bilinear')(x))
        if n_blocks >= 3:
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_shape, channels_per_group=16, n_blocks=4, nz_feat=100, bott_size=256, use_batch=True):
        super(Encoder, self).__init__()
        self.resnet_conv = ResNetConv(n_blocks=4)
        num_norm_groups = bott_size // channels_per_group
        # if opts.use_resnet50:
        self.enc_conv1 = nb.conv2d('group', config.RES_OUT, bott_size, stride=2, kernel_size=4, num_groups=num_norm_groups)
        # else:
        #     self.enc_conv1 = nb.conv2d('group', 512, bott_size, stride=2, kernel_size=4, num_groups=num_norm_groups)

        # 4 block resnet + conv + avpooling + fully connect

        nc_input = bott_size * (input_shape[0] // 64) * (input_shape[1] // 64)

        print('use batch norm', use_batch)
        if use_batch:
            self.enc_fc = nb.fc_stack(nc_input, nz_feat, 2, "batch")
        else:
            self.enc_fc = nb.fc_stack(nc_input, nz_feat, 2, None)
        self.nenc_feat = nc_input

        nb.net_init(self.enc_conv1)
        self.avgpool = nn.AvgPool2d(input_shape[0]//32, stride=1)
...
class ShapePredictor(nn.Module):
    ...
    def forward(self, feat):
        if self.use_sym_idx:
            ...
            delta_v = torch.Tensor(np.zeros((batch_size, self.num_verts, 3))).to(feat.device)
            feat = self.fc(feat)
            self.shape_f = feat
...
class ScalePredictor(nn.Module):
    ...
    def forward(self, feat):
        if not self.use_camera:
            ...
            scale[:, 0] = 0.
            return torch.Tensor(scale).to(feat.device)
        if self.norm_f0 != 0:
            off = 0.
        else:
            off = 1.
...
class TransPredictor(nn.Module):
    ...
    def forward(self, feat):
        trans = torch.Tensor(np.zeros((feat.shape[0], 3))).to(feat.device)
        f = torch.Tensor(np.zeros((feat.shape[0], 1))).to(feat.device)
        feat_xy = feat
        feat_z = feat
...
class MeshNet_img(nn.Module):
    def __init__(self,
                 ...
                 shape_init=None, return_feat=False, color=False,
                 faces_size=7774, use_batch=True):
        ...
        self.faces_size = faces_size
        self.encoder = Encoder(
            input_shape,
            channels_per_group=self.channels_per_group,
            n_blocks=4, nz_feat=nz_feat, bott_size=self.bottleneck_size, use_batch=use_batch)
        ...
        if self.color:
            self.color_net = ColorGen(dim_in=config.RES_OUT, im_size=input_shape, Nc=self.faces_size)
    ...
    def forward(self, img, masks=None, is_optimization=False, is_var_opt=False):
        ...
        if self.color: 
            # how can we relate the region belongs to the dog with the textures?
            Nd = self.color_net.Nd
            Nc = self.color_net.Nc
            col_sampling, col_selection = self.color_net(feat_resnet)
            col_sampling = F.softmax(col_sampling.view(-1, h*w, Nd), dim=1)
            col_selection = F.softmax(col_selection.view(-1, Nd, Nc), dim=1)
            # col_sampling: b 50176 15                
            # torch.Size([32, 3, 4096]) torch.Size([32, 4096, 15])
            # mat:  torch.Size([32, 3, 15]) col select:  torch.Size([32, 15, 7774])
            # torch.Size([32, 3, 7774])
        ...
        if self.return_feat:
            ...
            if self.color:
                return codes_pred, feat_resnet, feat_multiscale, textures
            else:
                return codes_pred, feat_resnet, feat_multiscale

        else:
            if self.color:
                return codes_pred, textures
            else: 
                return codes_pred
