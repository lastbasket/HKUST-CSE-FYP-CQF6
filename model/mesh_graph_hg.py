...
class MeshGraph_hg(nn.Module):
    def __init__(self, device, shape_family_id, number_channels, num_layers, betas_scale=False, shape_init=None
                 , local_feat=False, num_downsampling=0, render_rgb=True, color=False, return_feat=False, use_batch=True):
        '''
        ...
        self.meshnet = MeshNet_img(input_size, betas_scale=betas_scale, norm_f0=config.NORM_F0, nz_feat=config.NZ_FEAT,
                                   shape_init=shape_init, return_feat=True, faces_size = self.smal.faces.shape[0], color=self.color, use_batch=use_batch)
    ...
    def forward(self, img):
        if self.color:
            pred_codes, enc_feat, feat_multiscale, textures = self.meshnet(img)
        else:    
        ...
        pred_camera = torch.cat([scale_pred[:, [0]], torch.ones(scale_pred.shape[0], 2).to(device) * config.IMG_RES / 2],
                                dim=1)
        ...
        if self.color:
            if self.return_feat:
                return verts, joints, verts_re, pred_codes, textures, enc_feat
            else:
                return verts, joints, verts_re, pred_codes, textures
        else:
            if self.return_feat:
                return verts, joints, verts_re, pred_codes, enc_feat
            else:
