...
def batch_skew(vec, batch_size=None, opts=None):
    ...
    col_inds = torch.LongTensor([1, 2, 3, 5, 6, 7]).to(vec.device)
    indices = torch.reshape(torch.reshape(torch.arange(0, batch_size).to(vec.device) * 9, [-1, 1]) + col_inds, [-1, 1])
    ...
    res = torch.Tensor(np.zeros(out_shape[0])).to(vec.device)
...
def batch_rodrigues(theta, opts=None):
    ...
    eyes = torch.eye(3).unsqueeze(0).repeat([batch_size, 1, 1]).to(theta.device)
