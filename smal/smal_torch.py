...
from smal.batch_lbs import batch_rodrigues, batch_global_rigid_transformation
from smal.smal_basics import align_smal_template_to_symmetry_axis
...
class SMAL(nn.Module):
    def __init__(self, device, shape_family_id=1, dtype=torch.float):
        ...
        self.shape_cluster_cov = data['cluster_cov'][shape_family_id]
    ...
    def __call__(self, beta, theta, trans=None, del_v=None, betas_logscale=None, get_skin=True, v_template=None):
        ...
        if nBetas > 0:
            if del_v is None:
                v_shaped = v_template.to(beta.device) + torch.reshape(torch.matmul(beta, self.shapedirs[:nBetas,:].to(beta.device)), [-1, self.size[0], self.size[1]])
            else:
                v_shaped = v_template.to(beta.device) + del_v + torch.reshape(torch.matmul(beta, self.shapedirs[:nBetas,:].to(beta.device)), [-1, self.size[0], self.size[1]])
        else:
            if del_v is None:
        ...
        Jx = torch.matmul(v_shaped[:, :, 0], self.J_regressor.to(v_shaped.device))
        Jy = torch.matmul(v_shaped[:, :, 1], self.J_regressor.to(v_shaped.device))
        Jz = torch.matmul(v_shaped[:, :, 2], self.J_regressor.to(v_shaped.device))
        ...
        v_posed = torch.reshape(
            torch.matmul(pose_feature, self.posedirs.to(beta.device)),
            [-1, self.size[0], self.size[1]]) + v_shaped.to(beta.device)
        ...
        weights_t = self.weights.repeat([num_batch, 1]).to(beta.device)
        ...
        # Get joints:
        joint_x = torch.matmul(verts[:, :, 0], self.J_regressor.to(beta.device))
        joint_y = torch.matmul(verts[:, :, 1], self.J_regressor.to(beta.device))
        joint_z = torch.matmul(verts[:, :, 2], self.J_regressor.to(beta.device))
        ...
        if get_skin:
            return verts, joints, Rs, v_shaped.to(verts.device)
        else:
            return joints

    def verts2joints(self, verts):
        # Get joints:
