...
class NeuralRenderer(torch.nn.Module):
    ...
    def render_depth(self, verts, faces, cams):
        verts = self.proj_fn(verts, cams, offset_z=self.offset_z, norm_f=self.norm_f, norm_z=self.norm_z, norm_f0=self.norm_f0)
        
        depth = self.renderer.render_depth(verts, faces)
        return depth
