...
def main(args):
    ...
    model = MeshGraph_hg(device, args.shape_family_id, args.num_channels, args.num_layers, args.betas_scale,
                      ...
                      render_rgb=True, color=args.color, use_batch=False)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.color:
                logger.set_names(['Epoch', 'LR', 'PCK', 'IOU', 'PCK_re', 'IOU_re', 'PSNR', 'PSNR_re'])
            else:
    ...
    if args.evaluate:
        if args.color:
            pck, iou_silh, pck_by_part, pck_re, iou_re, psnr, psnr_re = run_evaluation(model, dataset_eval, data_loader_eval, device, args)
            print("Epoch: {:3d}, LR: {:6.5f}, PCK: {:6.4f}, IOU: {:6.4f}, PCK_re: {:6.4f}, IOU_re: {:6.4f}, PSNR: {:6.4f}, PSNR_re: {:6.4f}".format(epoch, lr, pck, iou_silh, pck_re, iou_re, psnr, psnr_re))
        else:
    ...
    for epoch in range(args.start_epoch, args.nEpochs):
        ...
        for step, batch in enumerate(tqdm_iterator):
            ...
            if args.w_dice_refine> 0:
                ...
                if args.color:
                    ...
                    c_loss_refine = color_loss(orig_img*seg, synth_rgb_refine[0]*synth_silhouettes.unsqueeze(1))
                    meters.update('loss_color', c_loss_refine.item())
                    loss += c_loss_refine
                else:
        ...
        if (epoch+1)%args.eval_freq==0:
            ...
            if args.color:
                pck, iou_silh, pck_by_part, pck_re, iou_re, psnr, psnr_re = run_evaluation(model, dataset_eval, data_loader_eval, device, args)
                print("Epoch: {:3d}, LR: {:6.5f}, PCK: {:6.4f}, IOU: {:6.4f}, PCK_re: {:6.4f}, IOU_re: {:6.4f}, PSNR: {:6.4f}, PSNR_re: {:6.4f}".format(epoch, lr, pck, iou_silh, pck_re, iou_re, psnr, psnr_re))
                logger.append([epoch, lr, pck, iou_silh, pck_re, iou_re, psnr, psnr_re])

                writer.add_scalar("pck", pck, epoch+1)
                writer.add_scalar("iou", iou_silh, epoch+1)
                writer.add_scalar("psnr", psnr, epoch+1)

                writer.add_scalar("pck_re", pck_re, epoch+1)
                writer.add_scalar("iou_re", iou_re, epoch+1)
                writer.add_scalar("psnr_re", psnr_re, epoch+1)

                is_best = (pck_re > best_pck and psnr_re > best_psnr and iou_re > best_iou)
                if pck_re > best_pck and (psnr_re > best_psnr) and (iou_re > best_iou):
                    best_pck_epoch = epoch
                best_pck = max(pck_re, best_pck)
                best_psnr = max(psnr_re, best_psnr)
                best_iou = max(iou_re, best_iou)
...
def run_evaluation(model, dataset, data_loader, device, args):
    ...
    for step, batch in enumerate(tqdm_iterator):
        with torch.no_grad():
            ...
            if args.color:
                ...
                preds['PSNR_re'] = Metrics.PSNR(synth_rgb_refine[0], orig_img, seg)
        ...
        if args.color:
            ...
            psnr_re[step * batch_size:step * batch_size + curr_batch_size] = preds['PSNR_re'].data.cpu().numpy()
    ...
    if args.color:
        return np.nanmean(pck), np.nanmean(acc_sil_2d), pck_by_part, np.nanmean(pck_re), np.nanmean(acc_sil_2d_re), np.nanmean(psnr), np.nanmean(psnr_re)
    else:
