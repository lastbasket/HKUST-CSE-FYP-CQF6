...
def main(args):
    ...
    dataset_train = BaseDataset(args.dataset, param_dir=args.param_dir,
                                is_train=False, use_augmentation=False, img_res=config.IMG_RES, field=True, img_idx=args.img_idx)
    data_loader_train = DataLoader(
        dataset_train, batch_size=args.batch_size, shuffle=False, num_workers=args.num_works)
    # set optimizer
    ...
    run_train(model, dataset_train, data_loader_train, device, args)
    return


def run_train(model, dataset, data_loader, device, args):
    ...
    pck_by_part = {group: np.zeros((len(dataset)))
                   for group in config.KEYPOINT_GROUPS}
    pck_by_part_re = {group: np.zeros((len(dataset)))
                      for group in config.KEYPOINT_GROUPS}
    ...
    batch_iterator = tqdm(data_loader, desc='Batch', total=len(data_loader))
    ...
    for epoch in epoch_iterator:
        ...
        for step, batch in enumerate(batch_iterator):
            ...
            if args.vis and ((epoch+1) == args.nEpochs):
                ...
                for i in range(preds['synth_xyz_re'].shape[0]):
                    ...
                    path_suffix = "{0}_{1}".format(path_parts[-2], path_parts[-1]).replace('.jpg', '')
                    ...
                    cv2.imwrite(os.path.join(vis_dir, f"{path_suffix}_mask.png"), np.clip(seg[i].permute(1, 2, 0).detach().cpu().numpy()*255, 0, 255).astype(np.uint8))
                    
                    ori = cv2.cvtColor(np.clip((orig_img[i]*seg[i]).permute(1, 2, 0).detach().cpu().numpy()*255, 0, 255).astype(np.uint8), cv2.COLOR_RGB2BGR)
                    cv2.imwrite(os.path.join(vis_dir, f"{path_suffix}_ori.png"), ori)
                    ori_wo_mask = cv2.cvtColor(np.clip(orig_img[i].permute(1, 2, 0).detach().cpu().numpy()*255, 0, 255).astype(np.uint8), cv2.COLOR_RGB2BGR)
                    cv2.imwrite(os.path.join(vis_dir, f"{path_suffix}_ori_wo_mask.png"), ori_wo_mask)
            ...
            c_loss_refine = color_loss(
                orig_img*seg, preds['synth_xyz_re']*preds['synth_silhouettes_re'])
            loss = c_loss_refine

            curr_batch_size = preds['synth_landmarks'].shape[0]
            smal_pose[step * batch_size:step * batch_size +
                      curr_batch_size] = preds['pose'].data.cpu().numpy()
            smal_betas[step * batch_size:step * batch_size + curr_batch_size,
                       :preds['betas'].shape[1]] = preds['betas'].data.cpu().numpy()
            smal_camera[step * batch_size:step * batch_size +
                        curr_batch_size] = preds['camera'].data.cpu().numpy()
        ...
        writer.add_scalar('loss', total_loss/len(batch_iterator), epoch+1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_idx', default=4, type=int)
